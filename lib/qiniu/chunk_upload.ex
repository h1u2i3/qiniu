defmodule Qiniu.ChunkUpload do
  ...
  def mkblk(module \\ Qiniu, stream, opts, uptoken, block_size) do
    headers =
    ...
    url = module.config()[:up_host] <> "/mkblk/#{block_size}"
    post_with_retry(url, stream, %{headers: headers})
  end

  def bput(module \\ Qiniu, stream, offset, ctx, uptoken, host \\ "") do
    headers = [
      content_type: "application/octet-stream",
      content_length: byte_size(stream),
      authorization: "UpToken " <> uptoken
    ...
    url =
      (host || module.config()[:up_host]) <>
        "/bput/#{ctx}/#{offset * @default_chunk_size}"

    post_with_retry(url, stream, %{headers: headers})
  end

  def mkfile(module \\ Qiniu, ctxs, size, opts, uptoken) do
    headers =
      Keyword.merge(opts[:headers] || [],
    ...
    url = module.config()[:up_host] <> "/mkfile/#{size}#{opts_string}"
